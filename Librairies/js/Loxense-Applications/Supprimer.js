function ModalSupprimer( Id, Libelle, Type, Localisation ){
    var Message = 'zzz';
    var Erreur = false;

    $.ajax({
        url: '../../../Loxense-Applications.php?Action=AJAX_Verifier_Associer',
        type: 'POST',
        dataType: 'json',
        data: $.param({'id': Id, 'libelle': Libelle, 'type': Type, 'localisation': Localisation}),
        async: false,
    ...
    $.ajax({
        ...
        success: function( reponse ) {
            ...
            $('#formSupprimer').submit( function( event ) { // Gère la soumission du formulaire.
                ...
                supprimerApplication( Id, Libelle, Type, Localisation );
            } );
        }
    });
}
...
function supprimerApplication( Id, Libelle, Type, Localisation ) {
    var total = $( '#totalOccurrences' ).text();
    total = Number(total) - 1;

    $.ajax({
        url: '../../../Loxense-Applications.php?Action=AJAX_Supprimer',
        type: 'POST',
        data: $.param({'id': Id, 'libelle': Libelle, 'type': Type, 'localisation': Localisation}),
        dataType: 'json',
        success: function( reponse ) {
