...
function trier( myElement, changerTri, chercher ) {
	...
	$.ajax({
		url: '../../../Loxense-EditionsActions.php?Action=AJAX_Trier',
		type: 'POST',
...
function chargerImpressionsCartoExcel( crs_id ) {
	$.ajax({
		url: '../../../Loxense-EditionsActions.php?Action=AJAX_Verifier_Impression_Excel',
		type: 'POST',
		...
		success: function(reponse) { // Le serveur n'a pas rencontré de problème lors de l'échange ou de l'exécution.
			if ( reponse['statut'] == 'success' ) {
				window.location.href = '../../../Loxense-EditionsActions.php?Action=AJAX_Charger_Impression_Excel&crs_id='+crs_id;
			} else {
...
function chargerImpressionsCartoWord( crs_id ) {
	$.ajax({
		url: '../../../Loxense-EditionsActions.php?Action=AJAX_Verifier_Impression_Word',
		type: 'POST',
		data: $.param({'crs_id': crs_id}),
		dataType: 'json', // le résultat est transmit dans un objet JSON
		...
		success: function(reponse) { // Le serveur n'a pas rencontré de problème lors de l'échange ou de l'exécution.
			if ( reponse['statut'] == 'success' ) {
				window.location.href = '../../../Loxense-EditionsActions.php?Action=AJAX_Charger_Impression_Word&crs_id='+crs_id;
			} else {
				afficherMessage( reponse['texteMsg'], reponse['statut'] );
			}
		}
...
function ModalChoixImpression( Id ) {
	...
	$.ajax({
		url: '../../../Loxense-EditionsActions.php?Action=AJAX_Libeller',
		type: 'POST',
		dataType: 'json',
		success: function( reponse ) {
			var Corps =
...
function genererImpressionCarto( crs_id, type_edition ) {
	$.ajax({
		url: '../../../Loxense-EditionsActions.php?Action=AJAX_Generer_Impression',
		type: 'POST',
...
function chargerImpressionsCarto( crs_id ) {
	$.ajax({
		url: '../../../Loxense-EditionsActions.php?Action=AJAX_Verifier_Impression',
		type: 'POST',
		data: $.param({'crs_id': crs_id}),
		dataType: 'json', // le résultat est transmit dans un objet JSON
		...
		success: function(reponse) { // Le serveur n'a pas rencontré de problème lors de l'échange ou de l'exécution.
			if ( reponse['statut'] == 'success' ) {
				window.location.href = '../../../Loxense-EditionsActions.php?Action=AJAX_Charger_Impression&crs_id='+crs_id;
