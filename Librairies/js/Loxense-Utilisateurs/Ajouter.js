...
function ajouterUtilisateur() {
	...
	$.ajax({
		url: '../../../Loxense-Utilisateurs.php?Action=AJAX_Ajouter',
		type: 'POST',
		data: $.param({'idn_login': idn_login, 'idn_super_admin': idn_super_admin, 'cvl_id': cvl_id,
			'ent_id': ent_id, 'ent_libelle': ent_libelle, 'cvl_label': cvl_label, 'idn_courriel': idn_courriel,
			'liste_profils': liste_profils, 'liste_entites': liste_entites, 'liste_gestionnaires': liste_gestionnaires,
			'liste_etiquettes': liste_etiquettes}), // les paramètres sont protégés avant envoi
			...
			if ( statut == 'success' ) {
				...
				if ( reponse[ 'droit_supprimer' ] == 1 ) {
					$('#IDN_' + reponse[ 'id' ] + ' .btn-supprimer').click(function(){
						ModalSupprimer( reponse[ 'id' ], ent_libelle, cvl_label, idn_login  );
					});
				}
			} else {
	...
	$.ajax({
		...
		success: function( reponse ) {
			...
			Corps = '<div class="zone_ajout_contextuel" id="zone_ajout_civilite">' +
				...
				'<div class="col-lg-10">' +
				'<input id="cvl_nom" class="form-control" type="text" autofocus required>' +
				'</div>' +
				'</div>' +
				'<div class="row">' +
		...
		success: function( reponse ) {
			...
			if ( reponse['is_super_admin'] == true ) {
				Corps += '<div class="row mb-3">' +
					'<label class="col-lg-3 form-check-label" for="ent_admin">' + reponse[ 'L_Administrator' ] + '</label>' +
					'<div class="col-lg-9">' +
			...
			success: function( reponse ) { // Le serveur n'a pas rencontré de problème lors de l'échange ou de l'exécution.
				...
				if ( statut == 'success' ) {
					...
					$('#liste-entites').append(
						...
						'   <label class="form-check-label">' + reponse['L_Administrator'] + '</label>' +
						'  </div>' +
						' </div>' +
						'</div>'
					);
