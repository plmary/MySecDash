	...
	$.ajax({
		...
		success: function( reponse ) {
			if ( app_id == '' ) {
				...
				var ete_id_dima_dsi = '';
				var ete_id_pdma_dsi = '';
				var scap_description_dima = '';
				var scap_description_pdma = '';
			} else {
				var Titre = reponse['L_Titre_Modifier'];
				var Bouton = reponse[ 'L_Modifier' ]

				var app_nom = reponse['Application'].app_nom;
				var app_nom_alias = (reponse['Application'].app_nom_alias != null ? reponse['Application'].app_nom_alias : '');
				var frn_id = reponse['Application'].frn_id;
				var app_hebergement = (reponse['Application'].app_hebergement != null ? reponse['Application'].app_hebergement : '');
				var app_niveau_service = (reponse['Application'].app_niveau_service != null ? reponse['Application'].app_niveau_service : '');
				var app_description = (reponse['Application'].app_description != null ? reponse['Application'].app_description : '');
				var sct_id = reponse['Application'].sct_id;
				var ete_id_dima_dsi = reponse['Application'].ete_id_dima_dsi;
				var ete_id_pdma_dsi = reponse['Application'].ete_id_pdma_dsi;
				var scap_description_dima = (reponse['Application'].scap_description_dima != null ? reponse['Application'].scap_description_dima : '');
				var scap_description_pdma = (reponse['Application'].scap_description_pdma != null ? reponse['Application'].scap_description_pdma : '');
			}
			...
			for (let element of reponse['Liste_Echelles']) {
				if (element.ete_id == ete_id_dima_dsi) {
					Selection = 'selected';
				} else {
					Selection = '';
				}
				Corps += '<option value="'+element.ete_id+'" ' + Selection + '>'+element.ete_nom_code+'</option>';
			}

			Corps += '</select>' +
				'</div>' + // .col-lg-2
				'<div class="col-lg-8">' +
				 '<textarea id="scap_description_dima" class="form-control" rows="3">'+ scap_description_dima + '</textarea>' +
				'</div>' + // .col-lg-8
				'</div>' + // .row
				...
				for (let element of reponse['Liste_Echelles']) {
					if (element.ete_id == ete_id_pdma_dsi) {
						Selection = 'selected';
					} else {
						Selection = '';
					}
			...
			Corps += '</select>' +
				 '</div>' + // .col-lg-2
				 '<div class="col-lg-8">' +
				  '<textarea id="scap_description_pdma" class="form-control" rows="3">'+ scap_description_pdma + '</textarea>' +
				 '</div>' + // .col-lg-8
				'</div>' + // .row
				'</div>' + // .border
