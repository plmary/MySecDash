			...
			if( statut == 'success' ){
				...
				if ( reponse[ 'droit_supprimer' ] == 1 ) {
					...
					$('.btn-supprimer').click(function(){
						var Id = $(this).attr('data-id');
						var Libelle = $('#TFR_'+Id).find('div[data-src="tfr_nom_code"]').find('span').text();
...
function ModalAjouterModifier( tfr_id = '' ){
	$.ajax({
		url: Parameters['URL_BASE'] + Parameters['SCRIPT'] + '?Action=AJAX_Libeller',
		type: 'POST',
		data: $.param({'tfr_id': tfr_id}), // les paramètres sont protégés avant envoi
		dataType: 'json',
		success: function( reponse ) {
			if ( tfr_id == '' ) {
				...
				tfr_nom_code = reponse['TypeFournisseur'][0].tfr_nom_code;
			}

			var Corps =
				'<div class="row">' +
				'<label class="col-lg-2 col-form-label" for="tfr_nom_code">' + reponse[ 'L_Nom' ] + '</label>' +
				'<div class="col-lg-10">' +
				'<input id="tfr_nom_code" class="form-control" type="text" value="'+ tfr_nom_code + '" required>' +
				'</div>' +
				'</div>';
			...
			$('#idModal').on('shown.bs.modal', function() {
				document.getElementById('tfr_nom_code').selectionStart = tfr_nom_code.length;
				$('#tfr_nom_code').focus();
			});
			...
			$('#formAjouterModifier').submit( function( event ) { // Gère la soumission du formulaire.
				...
				if ( tfr_id == '' ) {
					...
					ModifierTypeFournisseur( tfr_id );
				}
			} );
		}
	});
