$(function() {
	$("#onglet-administrateur").click(function() {
		...
		$.ajax({
			url: Parameters['URL_BASE'] + Parameters['SCRIPT'] + '?Action=AJAX_Tableau_Bord_Admin',
			type: 'POST',
	...
	$("#onglet-utilisateur").click(function() {
		...
		$.ajax({
			url: Parameters['URL_BASE'] + Parameters['SCRIPT'] + '?Action=AJAX_Tableau_Bord_Utilisateur',
			type: 'POST',
			//data: $.param({'s_ent_id': $("#s_ent_id").val()}),
			dataType: 'json',
